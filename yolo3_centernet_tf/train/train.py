...
from model.yolov3_centernet import yolov3_centernet
from train.create_label import CreatGroundTruth
...
if 0:  # reload model
	...
	# model_file = "model_data/model_data.ckpt-3200"
...
try:
	while True:
		...
		pred_center, pred_offset, pred_size, cls_loss, offset_loss, size_loss, total_loss, global_step, lr, summary, _ = sess.run(
			fetches, feed)
		...
		if batch_num % 10 == 0:
			...
			print(
				"global_step:{0} total_loss:{1:0.3f} cls_loss:{2:0.3f} offset_loss:{3:0.3f} size_loss:{4:0.3f}".format(
					global_step, total_loss, cls_loss, offset_loss, size_loss))
